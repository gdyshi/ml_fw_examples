...
def format_data_set():
    ...
    for file in file_list:
        ...
        while line:
            ...
            x, y = img.size
            if (x < int(listFromLine[2]) | x < int(listFromLine[4]) | y < int(listFromLine[3]) | y < int(
                    listFromLine[5])):
...
def get_image(image_lists, label_name, index, category):
    # 获取给定类别中所有图片的信息。
    ...
    # 获取图片的文件名。
    image_full_name = category_list[mod_index]
    return image_full_name


# 这个函数通过类别名称、所属数据集和图片编号获取经过Inception-v3模型处理之后的特征向量文件地址。
def get_bottlenect(image_lists, label_name, index, category):
    # 获取图片的文件名。
    image_full_name = get_image(image_lists, label_name, index, category)
    bottlenect_full_name = image_full_name.replace(INPUT_DATA, CACHE_DIR)
    return bottlenect_full_name + '.txt'
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    # 获取一张图片对应的特征向量文件的路径。
    label_lists = image_lists[label_name]
    bottleneck_full_name = get_bottlenect(image_lists, label_name, index, category)
...
def main(_):
    ...
    # format_data_set()
    # 读取所有图片。
    create_one_label_lists('dddd', TEST_PERCENTAGE, VALIDATION_PERCENTAGE)
